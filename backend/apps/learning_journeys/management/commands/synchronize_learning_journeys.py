...
from apps.learning_journeys.application.services import SynchronizeLearningJourneyService
from apps.learning_journeys.domain.models import LearningJourney


class Command(BaseCommand):
    help = "Synchronize governed learning journey projections without advancing policy-owned learning state."
    ...
    def handle(self, *args, **options):
        ...
        for journey in queryset:
            processed += 1
            if options["dry_run"]:
                self.stdout.write(f"dry-run synchronize journey={journey.id} status={journey.status}")
                continue
            try:
                before = (journey.status, journey.version)
                synced = SynchronizeLearningJourneyService().execute(journey_id=journey.id, actor=actor or journey.learner)
                if before != (synced.status, synced.version):
                    updated += 1
        ...
        self.stdout.write(f"processed={processed} updated={updated} failed={failed} dry_run={options['dry_run']}")
