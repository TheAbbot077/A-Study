...
Retrieval readiness is `NOT_INDEXED`, `INDEXING`, `INDEXED`, `STALE`, or `FAILED`, but retrieval completion no longer transitions a processing job directly to `READY_FOR_TEACHING`. PI-6D.5 requires a separate authoritative teaching-readiness evaluation that reconciles durable source, processing, human review, approval, immutable projection, controlled population, active retrieval generation, and complete provenance. `RetrievalIndexJob` identity is population + retrieval version + embedding version; chunk identity additionally includes proposal version and policy version. Replays update rather than duplicate projections and publish collection-, embedding-, index-, readiness-, and grounding-level summary events.
